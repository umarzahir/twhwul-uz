        ...
        <CardBody>
          <div className="-mx-2 overflow-x-auto px-2">
              ...
              {data.progressStatusColumns.map((col) => (
                <div key={col.title} className="rounded-lg flex flex-col">
                  <div className="rounded-lg text-center bg-[var(--color-brand-navy)] px-3 py-3 text-white">
                    <div className="text-[11px] font-semibold leading-tight">{col.title}</div>
                    <div className="mt-3 inline-flex items-center justify-center rounded-full bg-[#4A6FA5] px-4 py-2 text-lg font-semibold tabular-nums">{col.percent.toFixed(2)}%</div>
                  </div>
                  <div className="flex flex-col gap-3 pt-3 flex-1">
                    {col.items.map((it) => (
                      <div key={it.name} className="rounded-lg border border-[var(--color-border)] bg-white p-3 flex flex-col">
                        <div className="text-sm font-medium text-slate-600 text-center">{it.name}</div>
                        ...
                        <div className="mt-3 flex items-center justify-center gap-2">
                          {it.dots.map((d, idx) => (
                            <span
                              key={idx}
                              className={[
                                "inline-flex size-6 items-center justify-center rounded-full text-[11px] font-semibold text-white",
                                dotTone(d),
                              ].join(" ")}
                            >
                              {idx + 1}
          ...
          <CardBody>
            <ul className="space-y-3">
              {data.topLeaders.map((l) => (
                <li key={l.name} className="flex items-center justify-between gap-3 border-b border-[var(--color-border)] pb-3">
          ...
          <CardBody>
            <ul className="space-y-3">
              {data.recentActivitiesV2.map((a) => (
                <li key={a.title} className="flex items-start gap-3 border-b border-[var(--color-border)] pb-3">
                  <div className="rounded-full mt-2 w-1 h-1 bg-[#D62828]"></div>
                  <div className="min-w-0">
                    <div className="text-sm font-semibold text-slate-900">
                      {a.title}</div>
                    <div className="mt-1 text-xs text-slate-500">{a.subtitle}</div>
                  </div>
                  <div className="shrink-0 text-[11px] font-medium text-slate-400 ml-auto">{a.ago}</div>
                </li>
              ))}
            </ul>
          </CardBody>
